...
uniform float maskHE3x3[MASK_SIZE_3x3];
...
void main()
{
	...
	float z = texture(depthTex, texCoord)[0];
	//prof = - (n*f)/(f + prof *(n-f));
	float depth = - (1.0 * 50.0) / (50.0 + z * (1.0 - 50.0));
	...
	for (uint i = 0u; i < MASK_SIZE_3x3; i++) {
		...
		color += texture(colorTex, iidx, 0.0) * maskHE3x3[i];
	}
